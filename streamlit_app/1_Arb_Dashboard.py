...
with tab_bodega:
    ...
    if manual_pairs_bodega:
        ...
        for b_id, p_id, is_flipped, profit_threshold in manual_pairs_bodega:
...
with tab_myriad:
    ...
    with mcol1:
        ...
        myriad_markets = load_myriad_markets()
        myriad_results = [m for m in myriad_markets if myriad_search.lower() in m['name'].lower()] if myriad_search else []
    ...
    with mcol3:
        ...
        if st.button("Add Myriad Pair"):
            if myriad_slug and poly_id_myriad:
                save_manual_pair_myriad(myriad_slug, poly_id_myriad, is_flipped=0, profit_threshold_usd=25.0)
    ...
    if manual_pairs_myriad:
        ...
        for m_slug, p_id, is_flipped, profit_threshold in manual_pairs_myriad:
            ...
            with st.form(key=f"form_pair_myriad_{m_slug}_{p_id}"):
                ...
                is_flipped_new = c2.checkbox("Flipped", value=bool(is_flipped))
                if c3.form_submit_button("Update Pair"):
                    save_manual_pair_myriad(m_slug, p_id, int(is_flipped_new), float(new_threshold))
        ...
        for m in pending_myriad:
            ...
            with cols[1]:
                ...
                if st.button("Match", key=f"match_myriad_{m['market_id']}"):
                    if poly_id:
                        save_manual_pair_myriad(m["market_slug"], poly_id, 0, 25.0)
...
# Frequency Control UI is the same...
...
if st.button("Check All Manual Pairs for Arbitrage"):
    with st.spinner("Checking all pairs for arbitrage opportunities..."):
        ...
        if not manual_pairs_bodega: st.info("No manual Bodega pairs to check.")
            ...
            for i, (b_id, p_id, is_flipped, profit_threshold) in enumerate(manual_pairs_bodega, start=1):
                try:
